...
const { isMine, isLike, getAccessLevel } = require("../utils/accessLevel");
    ...
    try {
        ...
        if (req.fileUrl !== null) {
            ...
            post = await Post.create(
                { author: req.nickname, content, image: fileUrl },
                { transaction }
            );
        } else {
            // 파일이 첨부되지 않은 경우
            post = await Post.create(
                { author: req.nickname, content },
                { transaction }
            );
        }
    ...
    try {
        ...
        return res.status(200).json({
            ...
            post: {
                ...
                likesCount,
                commentCount,
                userId: post.User.dataValues.userId,
...
async function putGroupPost(req, res, next) {
    ...
    try {
        ...
        if (req.fileUrl !== null) {
            ...
            const fileUrl = req.fileUrl.join(", ");
            // 새로 등록된 이미지 주소로 업데이트
            modifiedPost = await post.update(
...
async function deleteGroupPost(req, res, next) {
    ...
    try {
        ...
        if (
            accessLevel === "viewer" ||
            accessLevel === null ||
            (accessLevel === "regular" && !isMine(user, post))
        ) {
            throw new EditPermissionError();
        }
...
async function putComment(req, res, next) {
    ...
    try {
        ...
        // 댓글 수정
        const { content } = req.body;
        const modifiedComment = await comment.update(
            { content },
            { transaction }
        );
...
async function deleteComment(req, res, next) {
    ...
    try {
        ...
        if (
            accessLevel === "viewer" ||
            accessLevel === null ||
            (accessLevel === "regular" && !isMine(user, comment))
        ) {
            throw new EditPermissionError();
        }
